...
void AvgTime(int process[], int n, int brust[],int quant,char name)
{
    ...
    if(name=='s')
    {
	printf("\n Time he spend on handling of lottery query is : %d",avg_turnaround);
...
int main()
{
    ...
    //for students 
    printf("ENTER SCHEDULING TIME : \n\n");
    int i;
    for(i=1;i<=x;i++)
    {
	printf("Enter arival time  : %d\t",i);
	scanf("%d",&process[i]);
        printf("\n");
    }
    
    printf("ENTER BRUST TIME OF LOTTERY: \n\n");
    for(i=1;i<=x;i++)
    {
       printf("Enter brust time of LOTTERY : %d\t",i);
    ...
    int quantum = 5;
    printf("time taken 10 to 15 minutes for handling query: \n\n");
    AvgTime(process, n, burst_time, quantum,'s'); 
    n = sizeof process1 / sizeof process1[0];
    printf("\n\n");
   printf("time arrive  10 to 15 minutes for handling query: \n\n");
    printf("\n\n\n");
